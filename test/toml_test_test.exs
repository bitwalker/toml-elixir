defmodule Toml.Test.TomlTestTests do
  @moduledoc """
  Runs the [toml-test](https://github.com/BurntSushi/toml-test) suite, adapted for TOML v0.5.0 and Elixir
  
  These tests are used as a validation for several TOML encoders/decoders, we're simply borrowing them!
  """
  use ExUnit.Case
  
  import Toml.Test.Assertions
  
  @toml_test_dir Path.join([__DIR__, "fixtures", "toml-test"])
  @valid_tests Path.wildcard(Path.join([@toml_test_dir, "valid", "*.toml"]))
  @num_valid length(@valid_tests)
  @invalid_tests Path.wildcard(Path.join([@toml_test_dir, "invalid", "*.toml"]))
  ...
  for {valid_test, line} <- Enum.with_index(@valid_tests), name = Path.basename(valid_test, ".toml") do
  ...
  for {invalid_test, line} <- Enum.with_index(@invalid_tests), name = Path.basename(invalid_test, ".toml") do
